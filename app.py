...
def status(request: Request, response: Response):  # pylint: disable=unused-argument
    ...
    return {"status": status_word, "plugins": list(plugins)}


@app.post("/api/v1/search")
@limiter.limit(settings.rate_limit)
async def search(
    request: Request, response: Response, sq: SearchQuery
...
def make_search_response(listings: list, cache_hit: bool, elapsed: timedelta) -> dict:
    return {
        "status": "ok",
        "length": len(listings),
        "cache_hit": cache_hit,
